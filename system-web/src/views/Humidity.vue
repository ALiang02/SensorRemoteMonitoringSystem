<template>
  <div class="humidity">
    ...
    <el-row type="flex">
      ...
      <el-col :span="8">
        <div id="humidity_pie" style="width: 100%; height: 900px" />
      </el-col>
    </el-row>
  </div>
</template>
  ...
  methods: {
    ...
    getNodes() {
      ...
      return this.$axios({
        ...
        console.log(this.humidity_data.values);
        for (let i = 0; i < this.humidity_data.values.length; i++) {
          for (let j = 0; j < this.humidity_data.values[i].length; j++) {
            let value = this.humidity_data.values[i][j][1];
            if (value <= 2) {
        ...
        for (let i = 0; i < this.humidity_data.sensors.length; i++) {
          this.legend_data[i] = this.humidity_data.sensors[i] + "号传感器";
          this.series_data[i] = {
    ...
    pie_draw() {
      this.pie_chart = echarts.init(document.getElementById("humidity_pie"));
    ...
    table_draw() {
      ...
      this.line_chart = echarts.init(document.getElementById("humidity"));
      // 绘制图表
