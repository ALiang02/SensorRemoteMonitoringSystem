...
export default {
  ...
  methods: {
    ...
    getNodes() {
      ...
      return this.$axios({
        ...
        for (let i = 0; i < this.smoke_data.sensors.length; i++) {
          this.legend_data[i] = this.smoke_data.sensors[i] + "号传感器";
          this.series_data[i] = {
    ...
    pie_draw() {
      this.pie_chart = echarts.init(document.getElementById("smoke_pie"));
    ...
    table_draw() {
      ...
      this.line_chart = echarts.init(document.getElementById("smoke"));
      // 绘制图表
